...
# Modified Distribution which uses Quantization Stubs
...
class QuantizedPtArgProj(nn.Module):
    r"""
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor]:
        x = self.quant(x)
        params_unbounded = [self.dequant(proj(x)) for proj in self.proj]
...
class QuantizedDistributionOutput(QuantizedOutput):
    r"""
...
class QuantizedNormalOutput(QuantizedDistributionOutput):
...
class QuantizedLaplaceOutput(QuantizedDistributionOutput):
    args_dim: Dict[str, int] = {"loc": 1, "scale": 1}
...
class QuantizedBetaOutput(QuantizedDistributionOutput):
...
class QuantizedGammaOutput(QuantizedDistributionOutput):
...
class QuantizedPoissonOutput(QuantizedDistributionOutput):
